...
class BaseDevice( object ):
    ...
    def checkRule( self ):
        ...
        for _index, _change in enumerate( self.rulechangelist ):
            if 0 == _change[2]:#延时到了
                _name = _item[0]
                _value = self.importFileParser[type][self.getVarDic()[_change[0]][0]]( _change[1] ) #根据类型进行转换
                self.addDataKeyValue( _name, _value )
    ...
    def unpackAppMsg( self, msgId, mes ):
        ...
        try:
            _mes = struct.unpack_from( self.__msg[msgId]['format'], mes )
        except struct.error, e:
...
if __name__ == '__main__':
    b = BaseDevice( 'base', 1 )
    #b.importVarint( r'./lc_variant.xml' )
    #print 'var dic', b.getVarDic()
    #print 'data dic', b.getDataDic()
    b.importMsg( r'./pddm5_message.xml' )
