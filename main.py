...
class TftpDialog(QDialog):
    ...
    def __init__(self):
        ...
        self.pddm5.deviceInit(msgFile = r'./config/pddm5_message.xml', log = r'./log/pddm5.log')
        self.Signal.connect(self.pddm5.fileParser) # 将fetch下来的tftp file文件名传递给Guest board
    ...
    def Ping(self):
        ...
        if self.IPCheck(self.ui.lineEdit_IP.text()) == True:
            ...
            if sys.platform.find("linux") == 0:
                child = subprocess.Popen(["ping", "-c", "4", self.ui.lineEdit_IP.text()],stdout=subprocess.PIPE)
            else:
                child = subprocess.Popen(["ping", "-n", "4", self.ui.lineEdit_IP.text()],stdout=subprocess.PIPE)     
    ...
    def Log(self, str):
        self.ui.textEdit_Log.append(time.asctime(time.localtime(time.time())) + ' ---- \n' + str)
